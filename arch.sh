...
pre_suffix=0
## do not place DATE before suffix
suffix=".*"
## used if pre_suffix is set to 1
...
create=0
## do not re-create (empty) file
gzip=
## do not use compression
hup=
## command to interrupt program writing to file
...
do
	case $opt in
		...
		D) dfmt=_%Y-%m-%d_%H.%M.%S ;;
		## use time as well as date
		f) dfmt=$OPTARG ;;
		## use supplied date format
		h) hup=$OPTARG ;;
		## command to run after mv
		s) pre_suffix=1 ;;
		## place date before suffix
		S) suffix=$OPTARG
		## place date before specified
		pre_suffix=1 ;;
		## suffix
		...
		Z) gzip=$OPTARG ;;
		## compress file with supplied cmd
		*) exit 5 ;;
